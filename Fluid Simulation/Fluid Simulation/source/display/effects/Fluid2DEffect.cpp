/********************************************************************
Fluid2DEffect.cpp: Encapsulates a 2D fluid effect being calculated on the
...
Fluid2DEffect::Fluid2DEffect() : pD3dGraphicsObj(nullptr), 
	timeStep(TIME_STEP),
...
bool Fluid2DEffect::Initialize(_In_ D3DGraphicsObject* d3dGraphicsObj, HWND hwnd) {
...
bool Fluid2DEffect::Render(FluidPropertyType_t fluidPropertyType) {
...
void Fluid2DEffect::ProcessEffect() {
...
void Fluid2D::Fluid2DEffect::AddObstacle(Vector2 &pos, float radius) {
...
void Fluid2D::Fluid2DEffect::AddDensity(Vector2 &pos, Vector2& amount, float radius) {
...
void Fluid2D::Fluid2DEffect::AddVelocity(Vector2 &pos, Vector2& amount, float radius) {
	SetImpulseBuffer(pos, amount, radius);
	mImpulseShader->Compute(pD3dGraphicsObj,&mVelocitySP[READ],&mVelocitySP[WRITE]);
	swap(mVelocitySP[READ],mVelocitySP[WRITE]);
}

void Fluid2D::Fluid2DEffect::Advect(ShaderParams *target) {
...
void Fluid2D::Fluid2DEffect::RefreshConstantImpulse() {
	int width,height;
	pD3dGraphicsObj->GetScreenDimensions(width,height);
...
void Fluid2D::Fluid2DEffect::CalculatePressureGradient() {
...
void Fluid2DEffect::SetGeneralBuffer() {
...
void Fluid2DEffect::SetDissipationBuffer(float dissipation) {
	D3D11_MAPPED_SUBRESOURCE mappedResource;
...
void Fluid2DEffect::SetImpulseBuffer(Vector2& point, Vector2& amount, float radius) {
	D3D11_MAPPED_SUBRESOURCE mappedResource;
